// 全局变量
let selectedAPIs = JSON.parse(localStorage.getItem('selectedAPIs') || '["tyyszy","dyttzy","bfzy","ruyi","iqiyi","myzy","yzzy","mtzy","ffzy","snzy","zy360","jisu","mozhua","mdzy","yinghua","lzi"]'); // 默认选中资源
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (!localStorage.getItem('hasInitializedDefaults')) {
        // 默认选中资源
        selectedAPIs = ["tyyszy","dyttzy","bfzy","ruyi","iqiyi","myzy","yzzy","mtzy","ffzy","snzy","zy360","jisu","mozhua","mdzy","yinghua","lzi"];
...
function resetSearchArea() {
    ...
    try {
        ...
        // 更新页面标题
        document.title = `闪电影视 - 免费在线视频搜索与观看平台`;
    } catch (e) {
...
async function importConfigFromUrl() {
    ...
    document.getElementById('confirmUrlImport').addEventListener('click', async () => {
        ...
        try {
            ...
            if (config.name !== '闪电影视-Settings') throw '配置文件格式不正确';
...
async function exportConfig() {
    ...
    config['name'] = '闪电影视-Settings';  // 配置文件名，用于校验
    ...
    // 将配置数据保存为 JSON 文件
    saveStringAsFile(JSON.stringify(config), '闪电影视-Settings_' + times + '.json');
}
